 ...
 * odomDebug.cpp
 *
 * The contents of this file were not written by any members of 333A*.
 * This is code from the publicly availible GitHub repository, odomDebug
 * by theol0403, found here: https://github.com/theol0403/odomDebug.
 *
 * The OdomDebug class is used for the tab on the screen of the brain
 * that shows the odometry position of the robot in the form of number
 * values and a moving circle on a picture of the field representing the
 * robot.
 *
...
{
constexpr QLength operator"" _tl(long double x) { return static_cast<double>(x) * tile; }
constexpr QLength operator"" _crt(long double x) { return static_cast<double>(x) * court; }
constexpr QLength operator"" _tl(unsigned long long int x) { return static_cast<double>(x) * tile; }
    ...
    : container(lv_obj_create(parent, NULL))
{
    ...
    cStyle.body.padding.inner = 0;
    cStyle.body.padding.hor = 0;
    cStyle.body.padding.ver = 0;

    lv_obj_set_style(parent, &cStyle);
    ...
    std::vector<std::vector<lv_style_t *>> tileData = {
        {&grey, &red, &grey, &grey, &blue, &grey},  {&red, &grey, &grey, &grey, &grey, &blue},
    ...
    lv_led_on(led);
    lv_obj_set_size(led, fieldDim / 15 * 2.5, fieldDim / 15 * 2.5);

    lv_style_copy(&ledStyle, &lv_style_plain);
    ledStyle.body.radius = LV_RADIUS_CIRCLE;
    ledStyle.body.main_color = LV_COLOR_MAKE(0, 255, 0);
    ledStyle.body.grad_color = LV_COLOR_MAKE(0, 255, 0);
    ...
    lineWidth = 3;
    lineLength = fieldDim / 4;

    lv_style_copy(&lineStyle, &lv_style_plain);
    lineStyle.line.width = 6;
    lineStyle.line.opa = LV_OPA_100;
    lineStyle.line.color = LV_COLOR_MAKE(0, 255, 0);
 ...
 */
void OdomDebug::setResetCallback(std::function<void()> callback) { resetFnc = callback; }
...
void OdomDebug::setData(state_t state, sensors_t sensors)
{
    ...
    double c_y = state.y.convert(court);
    ...
    std::string text =
        "X: " + std::to_string(std::round(state.x.convert(inch) * 1000) / 1000).substr(0, 5) +
        " in\n" +
        "Y: " + std::to_string(std::round(state.y.convert(inch) * 1000) / 1000).substr(0, 5) +
        " in\n" + "Theta: " +
        std::to_string(std::round(state.theta.convert(degree) * 1000) / 1000).substr(0, 5) +
        " deg\n" + "Left: " + std::to_string(sensors.left).substr(0, 4) + " deg\n" +
        "Right: " + std::to_string(sensors.right).substr(0, 4) + " deg\n";
    if (sensors.hasMiddle)
    {
        text = text + "Middle: " + std::to_string(sensors.middle).substr(0, 4) + " deg";
    }
...
lv_res_t OdomDebug::tileAction(lv_obj_t * tileObj)
{
    ...
    int x = num - y * 6;
    if (that->stateFnc)
        that->stateFnc({x * tile + 0.5_tl, y * tile + 0.5_tl, 0_deg});
    else
